...
type config_imp struct {
	Port   int    `json:"commentport,omitempty"` // port  на котором слушает микросервис новостей  по умолчанию 12345
...
func Help() string {
	ex, _ := os.Executable()
	var h string = `программма ` + tools.GetBase(ex) + ` является  микросервисем комментариев	 
	 использование  
	 ...
	 -config  --путь до конфиг файл с которого надо загружаться если параметр не указан то ищет config.json 
	 			ищет его в том же каталоге где и расположен исполняемый файл
	 -debug  --включить отладку
	формат конфиг файла  JSON.
	структура файла:
	{		
		"commentport" :  порт  прослушиваения (число ,необязательный ) по умолчанию 12345		
		"logdir":       каталог куда пишем лог ,пустой пишем в stdout
	 }
	 необходима переменная среды окружения :
	 "COMMENTDB=", формат  "postgres://<user>:<password>@<Host>:<Port>/<Database>
...
func New() *config {
	once.Do(func() {
		apiconfig = config{conf: config_imp{Port: 12346, Logdir: ""}}
	})
	return &apiconfig
}
...
func (c *config) String() string {
	ret := fmt.Sprintf("commentport: %d, logdir: %s ", c.conf.Port, c.conf.Logdir)
	return ret
}
